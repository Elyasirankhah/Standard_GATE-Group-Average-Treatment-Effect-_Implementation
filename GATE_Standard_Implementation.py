...
This implementation uses the standard X-learner algorithm:
...
BOOTSTRAP_REPS = 200           # Number of bootstrap replications for CIs
RANDOM_STATE = 42
...
def x_learner_cate_standard(X, t, y, ps, base_learner=None):
    """
    Standard X-learner for CATE estimation with propensity score weighting.
    
    Formula: τ(x) = e(x) * τ₀(x) + (1 - e(x)) * τ₁(x)
    ...
    5. Combine: τ(x) = e(x) * τ₀(x) + (1 - e(x)) * τ₁(x)
    """
    ...
    # Step 5: Combine using propensity score weighting (STANDARD X-LEARNER)
    tau0_pred = h0.predict(X)
    tau1_pred = h1.predict(X)
